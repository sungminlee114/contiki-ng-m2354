...
int adc_pushdata_process(uint8_t *data, size_t sz, meter_information_t* out) 
{
    adc_pushdata_t pdata = {0,};
    adc_pushdata_read(&pdata, data, sz);

    // printf("0x%"PRIx32" 0x%"PRIx32"\n", pdata.r.voltage.u32, pdata.r.vol_thd.u32);
    _float_to_char(pdata.r.voltage.f, out->V, 10);
    _float_to_char(pdata.r.current.f, out->I, 10);
    return 0;
}